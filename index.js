...
fetch(url)
  ...
  .then((data) => {
    if (data.code && data.code !== 200) {
      ...
      document.getElementById("description").textContent =
        `NASA's APOD server is currently down (${data.msg || "Internal Server Error"}). We can't fetch the image right now!`;
      return;
    }
  ...
  .catch((error) => {
    ...
    document.getElementById("description").textContent =
      "Could not reach the Astronomy Picture of the Day service.";
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderNotes() {
    ...
    let filteredNotes = notes.filter((note) => {
      const matchesFilter =
        currentFilter === "all" || note.type === currentFilter;
    ...
    bulletinContainer.innerHTML = filteredNotes
      .map(
        (note) => `
